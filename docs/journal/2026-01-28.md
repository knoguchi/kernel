...
- ~~**Large messages**: Shared memory regions for bulk transfer~~ Done (see shm.rs)
- ~~**More servers**: VFS, block device, network stack~~ VFS done (see below)

---

# 2026-01-28: VFS Server Implementation

## Overview

Implemented a user-space VFS (Virtual File System) server in Rust that provides file operations via IPC. The VFS server runs as task 3 and manages an in-memory filesystem (ramfs). Client programs (like init) communicate with VFS using IPC messages to open, read, write, and close files.

## Architecture

### Microkernel VFS vs Monolithic

In a monolithic kernel, VFS and filesystems run in kernel space with direct access to all memory. In our microkernel design:
...
Benefits:
- Fault isolation: VFS crash doesn't crash kernel
- Easier debugging: User-space debugging tools work
- Flexibility: Can replace/restart VFS without reboot

### RAM Filesystem (ramfs)

Simple in-memory filesystem with fixed limits (for kernel's 4KB frame allocation):
...
enum InodeType { Free, File, Directory }
...
On startup, VFS creates:
- `/` (root directory, inode 0)
- `/hello.txt` containing "Hello!\n"
- `/test.txt` containing "Test file.\n"
...
|-----|------|-------------|
| 100 | VFS_OPEN | Open file by path |
| 101 | VFS_CLOSE | Close file handle |
| 102 | VFS_READ | Read from file |
| 103 | VFS_WRITE | Write to file |

### VFS_OPEN

**Request**: Path embedded in message data (max 31 bytes)
```
data[0] byte 0: path length
data[0] bytes 1-7, data[1-3]: path bytes
```

**Reply**: tag = file handle (≥0) or error (<0)
...
```
data[0]: file handle
data[1]: max bytes to read
```

**Reply**: tag = bytes read (or error), data[0-3] = file content (up to 32 bytes)

### VFS_CLOSE

**Request**: data[0] = file handle
**Reply**: tag = 0 (success) or error

## Per-Client State

Each client has its own set of open files:
...
Client ID is derived from the IPC sender's task ID.

## Bug Fixed: sys_call Return Value

### Problem

VFS server worked correctly but init received wrong values:
```
[vfs] reply result=0x0000000000000000  (correct fd=0)
vfs_open returned: 0x0000000000000064   (wrong! 0x64 = VFS_OPEN tag)
```

### Root Cause

The C IPC wrapper `sys_call()` **returns** a new Message struct:

```c
// ipc.h
static inline Message sys_call(TaskId dest, const Message *msg) {
    Message reply;
    // ... asm fills reply from x0-x4 ...
    return reply;  // Returns NEW struct
}
```

But init.c was ignoring the return value:

```c
// init.c (BUGGY)
static long vfs_open(const char *path) {
    Message msg = { .tag = VFS_OPEN, ... };
    sys_call(VFS_SERVER, &msg);  // Return value IGNORED!
    return (long)msg.tag;         // Still VFS_OPEN (0x64)!
}
```
...
static long vfs_open(const char *path) {
    Message msg = { .tag = VFS_OPEN, ... };
    Message reply = sys_call(VFS_SERVER, &msg);  // Capture reply!
    return (long)reply.tag;  // Correct: fd or error
}
```

Same fix applied to `vfs_read()` and `vfs_close()`.
...
| File | Purpose |
|------|---------|
| `user/src/bin/vfs.rs` | VFS server: ramfs, IPC handlers |
| `user/src/lib.rs` | User-space Rust runtime: syscalls, IPC, console |
| `user/aarch64-kenix-user.json` | Target spec for user Rust binaries |

### Modified Files

| File | Changes |
|------|---------|
| `user/init.c` | Fixed sys_call return value handling, added VFS test |
| `kernel/src/sched/mod.rs` | Added `create_user_task_from_elf()` for VFS |
| `kernel/src/main.rs` | Create VFS server as task 3 |
| `Makefile` | Build Rust user-space programs |
...
--- VFS Test ---
[vfs] recv tag=0x0000000000000064
[vfs] OPEN
[vfs] reply result=0x0000000000000000
vfs_open returned: 0x0000000000000000
[vfs] recv tag=0x0000000000000066
[vfs] READ
[vfs] reply result=0x0000000000000007
vfs_read returned: 0x0000000000000007
Content: [Hello!
]
Raw bytes: 48 65 6c 6c 6f 21 0a
[vfs] recv tag=0x0000000000000065
[vfs] CLOSE
[vfs] reply result=0x0000000000000000
Closed file
[vfs] recv tag=0x0000000000000064
[vfs] OPEN
[vfs] reply result=0xfffffffffffffffe
Open /noexist.txt: ENOENT (OK)
--- VFS Test Done ---
```

- `vfs_open("/hello.txt")` returns fd=0 (success)
- `vfs_read(0, buf, 32)` returns 7 bytes: "Hello!\n"
- `vfs_close(0)` returns 0 (success)
- `vfs_open("/noexist.txt")` returns -2 (ENOENT)

## Future Work

- **Persistent storage**: Connect to a block device server
- **Multiple filesystems**: Mount points, filesystem type dispatch
- **Path resolution**: Handle `..`, symlinks
- **Permissions**: User/group ownership, mode bits
- **Large file transfer**: Use shared memory for files > 32 bytes
